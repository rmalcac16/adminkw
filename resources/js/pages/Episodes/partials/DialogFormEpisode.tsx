...
export function DialogFormEpisode({ anime, episode, open, setOpen }: Props) {
    const { __ } = useLang();
    const isEdit = !!episode;

    const form = useForm<{ number: number | string }>({
        number: isEdit ? (episode?.number ?? 1) : 1,
    });

    useEffect(() => {
        if (episode) {
            form.setData('number', episode.number);
        } else {
            form.reset();
        }
    }, [anime, episode]);
            ...
            <DialogContent>
                <form onSubmit={handleSubmit}>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {isEdit ? __('episodes.actions.edit_description', { number: episode.number }) : __('episodes.actions.create_description')}
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => {
                                    ...
                                    form.setData('number', value === '' ? '' : Number(value));
                                }}
